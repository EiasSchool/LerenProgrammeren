...
def getInvestorsCuts(profitGold, investors):
    interesting_investors = getInterestingInvestors(investors)
    return [round((investor["profitReturn"] / 100) * profitGold, 2) for investor in interesting_investors]

def getAdventurerCut(profitGold, investorsCuts, fellowship):
    leftoverGold = max(profitGold - sum(investorsCuts), 0.0)  # Voorkom negatieve waarde
    return round(leftoverGold / fellowship, 2) if fellowship > 0 else 0.0
